...
class Metropolis():
    '''
    Sampling from a certain probability distribution :math:`p (x) = \\frac {\\hat {p} (x)} {Z_p}`.
    The normalization constant satisfies :math:`Z_p = \\int \\hat {p} (x) dx `.
    Samples from :math:`p (x)` even if :math:`Z_p` is not known by the Metropolis method.
        ...
        with tqdm(total=sample_size) as bar:
            for i in range(sample_size * self.sample_rate):
                ...
                if i % self.sample_rate == 0:
                    samples.append(x)
                    post_str = f'{x}'
                    bar.set_description_str('MCMC(Metropolis):')
